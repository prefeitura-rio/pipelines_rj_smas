...
def get_datametrica_credentials():
    ...
    try:
        url = get_secret(constants.DATAMETRICA_PATH.value, "URL")
        token = get_secret(constants.DATAMETRICA_PATH.value, "TOKEN")
...
@task
def convert_agendamentos_to_dataframe(agendamentos: List[Agendamento]):
    """
