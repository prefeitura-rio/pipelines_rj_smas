...
INNER JOIN `rj-smas.protecao_social_cadunico.condicao_rua` cond_rua 
  ON cond_rua.id_membro_familia = tr.id_membro_familia 
  	AND cond_rua.data_particao = tr.data_particao
LEFT JOIN `rj-smas.protecao_social_cadunico.identificacao_primeira_pessoa` pp
  ON pp.id_membro_familia = tr.id_membro_familia
    ...
    AND pp.estado_cadastral = "Cadastrado"
...
total_data_cras_cres AS (
  SELECT
      tr.data_particao,
      COUNT(1) AS total_membros_cras,
      SUM(COUNT(DISTINCT tr.id_membro_familia)) OVER(PARTITION BY tr.data_particao) AS total_membros,
  FROM `rj-smas.protecao_social_cadunico.trabalho_remuneracao` tr
  INNER JOIN `rj-smas.protecao_social_cadunico.condicao_rua` cond_rua 
  	ON cond_rua.id_membro_familia = tr.id_membro_familia 
  	AND cond_rua.data_particao = tr.data_particao
